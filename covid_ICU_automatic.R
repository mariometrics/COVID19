...
for (i in 1:20) {
  ...
  Regioni[[i]] = Regioni[[i]][Regioni[[i]]$terapia_intensiva>0,]
  ritardo[i] = nrow(df[df$codice_regione==3,]) - nrow(Regioni[[i]])
  Regioni[[i]]$data = Regioni[[i]]$data - ritardo[i]
  
  ### Last percentage growth 
  if (nrow(Regioni[[i]]) >= 2) {
    gr_today[i] = (Regioni[[i]]$terapia_intensiva[nrow(Regioni[[i]])] - Regioni[[i]]$terapia_intensiva[nrow(Regioni[[i]])-1])/(Regioni[[i]]$terapia_intensiva[nrow(Regioni[[i]])-1])*100
    gr_today_text[i] = sprintf("%s: +%g%%\n", nomi_regioni[i], round(gr_today[i], digits = 2))
  }
  
  ### Fit Logistic & Gompertz
  if (nrow(Regioni[[i]]) > 40) {
    count[i] = i
    print(i)
    gomp[[i]] = nls(Regioni[[i]]$terapia_intensiva ~ SSgompertz(Regioni[[i]]$data, a, b, c), data = Regioni[[i]])
    logit[[i]] = nls(Regioni[[i]]$terapia_intensiva ~ SSlogis(Regioni[[i]]$data, a, b, c), data = Regioni[[i]])
    print(i)
    
    ### RMSE & goodness of fit
    pred_logi[[i]] = predict(logit[[i]])
    pred_gomp[[i]] = predict(gomp[[i]])
    rmse_logit[i] = rmse(Regioni[[i]]$terapia_intensiva,pred_logi[[i]])
    rmse_gomp[i] = rmse(Regioni[[i]]$terapia_intensiva,pred_gomp[[i]])
    r2_logit[i] = R2_Score(pred_logi[[i]],Regioni[[i]]$terapia_intensiva)
    r2_gomp[i] = R2_Score(pred_gomp[[i]],Regioni[[i]]$terapia_intensiva)
    ...
    gr[[i]] = diff(Regioni[[i]]$terapia_intensiva)/Regioni[[i]]$terapia_intensiva[1:nrow(Regioni[[i]])-1]
    gr_mean[i] = mean(gr[[i]])
    dt[i] = log(2,exp(1))/gr_mean[i]
    dt_today[i] = (Regioni[[i]]$data[nrow(Regioni[[i]])] - Regioni[[i]]$data[nrow(Regioni[[i]])-1])*log(2)/(log(Regioni[[i]]$terapia_intensiva[nrow(Regioni[[i]])]/Regioni[[i]]$terapia_intensiva[nrow(Regioni[[i]])-1]))
...
for (t in as.POSIXlt(date,format = "%Y-%m-%d")$yday) {
  aggr_it[t-min(df$data)] = sum(df$terapia_intensiva[df$data == t]) 
  if (length(aggr_it) >= 2) {
...
### Plot
pdf('./plot/plot_gomp_log_Region_ICU_auto.pdf',height=8, width=16)

par(xpd = T, mar = par()$mar + c(1,1,1,18))

plot(Regioni[[3]], lwd = 4, log = "y",
     xlim = c(min(Regioni[[3]]$data), end_ep/1.3),
     ylim = c(min(Regioni[[11]]$terapia_intensiva),max(Regioni[[3]]$terapia_intensiva)+mean(Regioni[[3]]$terapia_intensiva)),
...
legend(end_ep/1.3+2,max(Regioni[[3]]$terapia_intensiva)*5,
       legend = legend,
       col = color_, 
       lty = lty, 
       pch= pch, 
...
#text(end_ep/2+2,max(Regioni[[3]]$terapia_intensiva)/exp(4.4), paste(DT_text, collapse = ""), col = 1, lwd = 2, pos = 4)

text(end_ep/1.3+2,max(Regioni[[3]]$terapia_intensiva)/exp(5.7), paste("Top 5 Perc. Growth ", date[length(date)]), col = 1, lwd = 2, pos = 4)
text(end_ep/1.3+2,max(Regioni[[3]]$terapia_intensiva)/exp(6.9), paste(top5gr, collapse = ""), col = 1, lwd = 2, pos = 4)

text(min(df$data),max(Regioni[[3]]$terapia_intensiva/0.8), "Last 10 days Perc. growth all over Italy",col = 1, lwd = 2, pos = 4)
text(min(df$data),max(Regioni[[3]]$terapia_intensiva/7.7), paste(last5gr_it, collapse = ""),col = 1, lwd = 2, pos = 4)

text(max(df$data)-12,max(Regioni[[3]]$terapia_intensiva)/exp(4.2), paste("Saturation Deaths (not a reliable forecast) ", date[length(date)]), col = 1, pos = 4, cex = 0.9)
text(max(df$data)-12,max(Regioni[[3]]$terapia_intensiva)/exp(6.2), paste(saturation_text, collapse = ""),col = 1, pos = 4, cex = 0.8)
